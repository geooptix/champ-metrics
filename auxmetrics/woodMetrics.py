...
def visitLWDMetrics(visitMetrics, visit, channelUnits, largeWoodyPieces, largeWoodyDebris, woodyDebrisJams, jamHasChannelUnits):
...
def getDebrisVolumeForVisitAndChannelUnits_2014(channelUnitIDs, isWet, largeWoodyPieces):
    ...
    return [numpy.pi * numpy.math.pow((p["value"]["DiameterM"] / 2.0), 2) * p["value"]["LengthM"] for p in pieces]
...
def channelUnitLWDMetrics(channelUnitMetrics, largeWoodyPieces):

    for c in channelUnitMetrics:
        channelUnitID = c["ChannelUnitID"]
        if largeWoodyPieces is None:
            c["LargeWoodyPiecesCount"] = None
        else:
            c["LargeWoodyPiecesCount"] = [p for p in largeWoodyPieces["values"] if p["value"]["ChannelUnitID"] is not None and p["value"]["ChannelUnitID"] == channelUnitID].__len__()



def tier1LWDMetrics(tier1Metrics, visit, channelUnits, largeWoodyPieces, largeWoodyDebris, woodyDebrisJams, jamHasChannelUnits):
